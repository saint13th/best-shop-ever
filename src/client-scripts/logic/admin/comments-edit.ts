...
editButtons.forEach((element) => {
  element.addEventListener('click', async (event) => {
    try {
      ...
      const commentId = (event.target as HTMLElement).getAttribute(
        'data-commentId',
      );
      const wrapper = (event.target as HTMLElement).closest('tr');
      const userName = (
        wrapper.querySelector('.comment-userName') as HTMLInputElement
      ).value;
      const text = (wrapper.querySelector('.comment-text') as HTMLInputElement)
        .value;
  ...
  element.addEventListener('click', async (event) => {
    try {
      ...
      const { result, error } = await fetchService.deleteRequest({
        url: `/admin/comments/${productId}/${commentId}`,
        params: {},
      });
